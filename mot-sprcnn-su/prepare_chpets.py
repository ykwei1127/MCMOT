...
def df2coco(df, video_dir):
    ...
    images = []
    annotations = []
    if video_dir.stem == "c001" or video_dir.stem == "c006":
        imgW, imgH = 768, 576
    else:
        imgW, imgH = 720, 576
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pets_train', type=Path, default='../PETS09-S2L1/dataset/train')
    parser.add_argument('--pets_validation', type=Path, default='../PETS09-S2L1/dataset/validation')
    ...
    assert args.pets_train.exists()
    assert args.pets_validation.exists()
    ...
    for gt_path in args.pets_train.glob('**/**/gt/gt.txt'):
        df = pd.read_csv(gt_path, header=None)
        df.columns = ['fid', 'tag', 'x', 'y', 'w', 'h', 'conf', 'xx', 'yy', 'zz']
        imgs, anns = df2coco(df, gt_path.parent.parent)
        pets_train_data['images'].extend(imgs)
        pets_train_data['annotations'].extend(anns)
    with open(args.jsons / 'pets_train.json', 'w') as f:
        json.dump(normalize_ids(pets_train_data), f)
    print('pets_train: {} {}'.format(len(pets_train_data['images']), len(pets_train_data['annotations'])))

    pets_validation_data = {
        'images': [],
        'annotations': [],
        'categories': [{'id': 1, 'name': 'vehicle'}],
    }
    for gt_path in args.pets_validation.glob('**/**/gt/gt.txt'):
        df = pd.read_csv(gt_path, header=None)
        df.columns = ['fid', 'tag', 'x', 'y', 'w', 'h', 'conf', 'xx', 'yy', 'zz']
        imgs, anns = df2coco(df, gt_path.parent.parent)
        pets_validation_data['images'].extend(imgs)
        pets_validation_data['annotations'].extend(anns)
    with open(args.jsons / 'pets_validation.json', 'w') as f:
        json.dump(normalize_ids(pets_validation_data), f)
    print('pets_validation: {} {}'.format(len(pets_validation_data['images']), len(pets_validation_data['annotations'])))
    
    chpets = {
        'images': [*ch_data['images'], *pets_train_data['images']],
        'annotations': [*ch_data['annotations'], *pets_train_data['annotations']],
        'categories': [{'id': 1, 'name': 'person'}],
    }
    with open(args.jsons / 'chpets.json', 'w') as f:
        json.dump(normalize_ids(chpets), f)
    print('chpets: {} {}'.format(len(chpets['images']), len(chpets['annotations'])))