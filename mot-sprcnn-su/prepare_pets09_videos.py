...
if __name__ == '__main__':
    ...
    # parser.add_argument('--pets09_train', type=Path, default='../PETS09-S2L1/dataset/train')
    # parser.add_argument('--pets09_validation', type=Path, default='../PETS09-S2L1/dataset/validation')
    parser.add_argument('--pets09_test', type=Path, default='../PETS09-S2L1/dataset/test')
    # parser.add_argument('--pets09_temp', type=Path, default='../PETS09-S2L1/dataset/temp')
    parser.add_argument('--videos', type=Path, default='./videos/')
    args = parser.parse_args()

    # assert args.pets09_train.exists()
    # assert args.pets09_validation.exists()
    assert args.pets09_test.exists()

    # Prepare videos
    # args.videos.mkdir(parents=True)
    # (args.videos / 'campus_train').mkdir()
    # (args.videos / 'campus_validation').mkdir()
    (args.videos / 'pets09_test').mkdir()
    ...
    for video_dir in args.pets09_test.glob('*/*'):
        video_name = video_dir.stem
        paths = sorted(list(video_dir.glob('imgs/*.jpg')))
        fids = [int(p.stem) for p in paths]
        df_imgs = pd.DataFrame({'fid': fids, 'path': paths})
        df_imgs.to_csv(args.videos / 'pets09_test' / f'{video_name}.imgs', index=None)
        if (video_dir / 'gt' / 'gt.txt').exists():
            df_true = pd.read_csv(video_dir / 'gt' / 'gt.txt', header=None)
            df_true.columns = ['fid', 'tag', 'x', 'y', 'w', 'h', 'conf', 'xx', 'yy', 'zz']
            df_true.to_csv(args.videos / 'pets09_test' / f'{video_name}.gt', index=None)
