...
def prepare_data():
    ...
    for scene_dir in os.listdir(INPUT_DIR):
        if scene_dir in ["Dataset1", "Dataset2", "Dataset3", "Dataset4"]:
        # if scene_dir in ["Dataset1"]:
...
def exp_2_visualization(camera, camera_dir, obj_id, track, feat, weight):
    print(camera)
    print(camera, camera_dir)
    if not os.path.exists("exp"):
        os.mkdir("exp")
    if not os.path.exists(f"exp/{camera}"):
        os.mkdir(f"exp/{camera}")
    video_path = os.path.join(INPUT_DIR, camera_dir.split("/")[-2], camera, "imgs")
    for i in range(len(track.frame_list)):
        frame_path = os.path.join(video_path, f'{str(track.frame_list[i]).zfill(5)}.jpg')
        img = cv2.imread(frame_path)
        box = track.box_list[i]
        img = img[box[1]:box[1] + box[3], box[0]:box[0] + box[2]]
        cv2.imwrite(f"exp/{camera}/id-{obj_id}_f-{track.frame_list[i]}_w-{weight[i]}.jpg", img)

def exp_3_4_visualization(exp, camera, camera_dir, obj_id, track, sumsu, best_idx):
    if camera == "c038" and obj_id == 4:
        print(camera, obj_id)
        if not os.path.exists(exp):
            os.mkdir(exp)
        if not os.path.exists(f"{exp}/{camera}"):
            os.mkdir(f"{exp}/{camera}")
        if not os.path.exists(f"{exp}/{camera}/use"):
            os.mkdir(f"{exp}/{camera}/use")
        if not os.path.exists(f"{exp}/{camera}/nouse"):
            os.mkdir(f"{exp}/{camera}/nouse")
        video_path = os.path.join(INPUT_DIR, camera_dir.split("/")[-2], camera, "imgs")
        for i in range(len(track.frame_list)):
            if i in best_idx:
                frame_path = os.path.join(video_path, f'{str(track.frame_list[i]).zfill(5)}.jpg')
                ...
                box = track.box_list[i]
                img = img[box[1]:box[1] + box[3], box[0]:box[0] + box[2]]
                cv2.imwrite(f"{exp}/{camera}/use/id-{obj_id}_f-{track.frame_list[i]}_su-{sumsu[i]}.jpg", img)
            else:
                frame_path = os.path.join(video_path, f'{str(track.frame_list[i]).zfill(5)}.jpg')
                img = cv2.imread(frame_path)
                box = track.box_list[i]
                img = img[box[1]:box[1] + box[3], box[0]:box[0] + box[2]]
                cv2.imwrite(f"{exp}/{camera}/nouse/id-{obj_id}_f-{track.frame_list[i]}_su-{sumsu[i]}.jpg", img)
        exit()
...
def get_feature_dict(data, camera_dirs):
    ...
    for camera_dir in camera_dirs:
        ...
        # print(camera_averge_total_uncertainty)
        ### the average total uncertainty of a camera ###
        for obj_id in tracks:
            track = tracks[obj_id]
            if EXPSU:
                ...
                ### exp3: select the feature of total uncertainty that is smaller than the average of the tracklet###
                    ...
                    feat = torch.tensor(np.array(track.feature_list)[best_idx])
                    # exp_3_4_visualization("exp3", camera, camera_dir, obj_id, track, sumsu, best_idx)
                ### exp4: select the feature of total uncertainty that is smaller than the average total uncertainty of a camera###
                    ...
                    feat = torch.tensor(np.array(track.feature_list)[best_idx])
                    exp_3_4_visualization("exp4", camera, camera_dir, obj_id, track, sumsu, best_idx)
                ### exp5: feature weights by y=exp(-x)###
                elif EXPSU == 5:
                    box = np.array(track.box_list)
                    wh = box[:, 2:]
                    su = track.su_list
                    ...
                    sumsu = np.sum(resu, axis=1)
                    # print(sumsu)
                    # print(np.exp(-1*sumsu*(1/camera_averge_total_uncertainty)))
            ...
            # feature_dict[camera][obj_id] = mean
###################驗證實驗####################
    #         if camera == "c038" and obj_id == 4:
    #             exp_list.append(total_feat)
    #             exp_2_visualization(camera, camera_dir, obj_id, track, feat, weight)
    # exit()
###################驗證實驗####################
    return feature_dict
