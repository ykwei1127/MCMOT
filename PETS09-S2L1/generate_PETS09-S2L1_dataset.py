...
def move_videos():
    ...
    for camera_path in dataset_path.glob("*/*"):
        ...
        if not os.path.isdir(gt_path):
            os.mkdir(gt_path)
        for frame_path in camera_path.glob("frame*"):
            frame_name = frame_path.stem.split("_")[-1]
            source = frame_path
            destination = os.path.join(imgs_path, f"{frame_name}.jpg")
            shutil.move(source, destination)

def get_labels():
    base_path = Path("dataset/temp")
    label_dir = Path("label")
    for camera_path in base_path.glob("*/*"):
        camera_id = camera_path.stem
        gt_path = os.path.join(camera_path, "gt", "gt.txt")
        label_file = os.path.join(label_dir, f"{camera_id}.txt")
