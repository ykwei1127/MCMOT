...
from utils.mct     import build_model, build_model_2
...
def read_features_file(file):
    ...
    for line in f.readlines():
        ...
        if WITH_SU:
            ...
            ts = float(l[10])
            gps = list(map(float, l[11:13]))
            features = np.array(list(map(float, l[13:])), np.float32)
        else:
            ts = float(l[6])
...
def exp_visualization(camera, camera_dir, obj_id, track, feat, weight):
    print(camera)
    ...
    for i in range(len(track.frame_list)):
        frame_path = os.path.join(video_path, f'{str(track.frame_list[i]).zfill(4)}.jpg')
...
###################驗證實驗####################
    #         if camera == "c006" and obj_id == 169:
    #             exp_list.append(total_feat)
    #             exp_visualization(camera, camera_dir, obj_id, track, feat, weight)
    #         if camera == "c007" and obj_id == 112:
    #             exp_list.append(total_feat)
    #             exp_visualization(camera, camera_dir, obj_id, track, feat, weight)
    #         if camera == "c008" and obj_id == 183:
    #             exp_list.append(total_feat)
    #             exp_visualization(camera, camera_dir, obj_id, track, feat, weight)
    #         if camera == "c009" and obj_id == 288:
    #             exp_list.append(total_feat)
    #             exp_visualization(camera, camera_dir, obj_id, track, feat, weight)
    # aff = cosine(exp_list[0], exp_list[1].T)
    # print(aff)
    # aff = cosine(exp_list[0], exp_list[2].T)
    # print(aff)
    # aff = cosine(exp_list[0], exp_list[3].T)
    # print(aff)
    # exit()
###################驗證實驗####################
    return feature_dict
